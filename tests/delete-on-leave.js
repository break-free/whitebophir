...
function testBoardDelete(browser) {
    var page = browser.url(SERVER + '/boards/delete-on-leave')
        .waitForElementVisible('.tool[title ~= Pencil]') // pencil
    page = testDeleteOnRefreshWithOneUser(page);

    // test hideMenu
    browser.url(SERVER + '/boards/delete-on-leave?hideMenu=true').waitForElementNotVisible('#menu');
    browser.url(SERVER + '/boards/delete-on-leave?hideMenu=false').waitForElementVisible('#menu');
